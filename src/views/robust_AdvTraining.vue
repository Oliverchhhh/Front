...
import VerticalSteps from "../components/VerticalSteps.vue" // Import VerticalSteps
...
export default {
    name:"robust_advTraining", // Updated name for clarity
    components:{
        /* 注册组件 */
    navmodule: navmodule,
    ...
    VerticalSteps, // Register VerticalSteps
    },
    data(){
        return{
            htmlTitle:"CNN对抗训练",
        ...
        '$route': {
            ...
            handler(to) {
                this.setProgressStepsByRoute();
                 // Force update after route change and state update
                this.$nextTick(() => {
                    this.$forceUpdate();
                });
            }
    ...
    created() {
        ...
        // Removed initial setProgressStepsByRoute call here, handled by watcher
    },
    beforeDestroy() {
    ...
    mounted() {
        // 确保在组件挂载后设置正确的进度条状态 (watcher with immediate: true handles this)
        this.$nextTick(() => {
             this.setProgressStepsByRoute(); // Keep this call if needed for initial load before watcher runs
...
/* Added styles for layout - Copy from advAttackDefense.vue */
.main-container {
    display: flex;
    width: 100%;
    margin-bottom: 40px;
...
padding: 0px;
width: 1200px; /* Ensure width is consistent */
/* height: 824px; /* Consider using min-height or height: auto */
min-height: 824px;
background: #FFFFFF;
...
.datasetSelected{
    ...
    align-self: stretch;
    flex-grow: 0;
    margin-left: 48px; /* Add left margin to align with other inputs */
}
.modelSelected{
    display: flex;
    flex-direction: column;
    ...
    align-self: stretch;
    flex-grow: 0;
     margin-left: 48px; /* Add left margin */
...
.inputdiv {
    padding-left: 0px; /* Remove potential extra padding if needed */
}
.matchedDes{
    display: flex;
    flex-direction: column;
...
.conclusion_info{
    ...
    padding: 20px;
    gap: 15px 25px; /* Adjust vertical gap */
    margin-bottom: 20px;
    width: 1080px;
    /* gray-7 */
...
/* 结果文字样式 */
.resultext{
    width: 960px; /* Match container width */
    /* height: 22px; */
...
#rdeva{
    ...
    width: 1000px; /* Match echart width */
    /* height: 414px; */
    height: auto;
    /* Inside auto layout */
...
.mainParamNameNotop {
  margin-bottom: 16px;
  margin-left: 48px; /* Align with other inputs */
  font-family: 'HONOR Sans CN';
  font-weight: 500;
  font-size: 18px;
}
.mainParamName {
    margin-bottom: 16px;
    margin-top: 32px; /* Add consistent top margin */
    margin-left: 48px; /* Align with other inputs */
    font-family: 'HONOR Sans CN';
    font-weight: 500;
    font-size: 18px;
...
/* Add styles for labelSelection if needed */
.labelSelection {
...
.labelSelection .labelselected {
    background-color: #1890ff;
    color: #fff;
    border-color: #1890ff;
}

.labelSelection .labelunselected {
    background-color: #f5f5f5;
    color: rgba(0, 0, 0, 0.65);
}

.labelSelection button:hover {
    opacity: 0.8;
}
