...
export default {
    ...
    data() {
        return {
            ...
            currentMainStep: 1, // Model
            currentSubStep: 0,  // Adversarial Attack Defense
        }
    },
    watch: {
    ...
    created() {
       ...
       if (this.modelInfo[this.selectedModel] && this.modelInfo[this.selectedModel].name == "Vicuna-7B-v1.1"){
            this.showmethodInfo = [[{
                name: "LLM-Attacks",
                description: "LLM Attacks算法用于构建针对各种大型语言模型(LLM)的对抗攻击...",
            }]]
        } else {
    ...
    methods: {
        ...
        changeModel(index) {
            ...
            if (this.modelInfo[this.selectedModel] && this.modelInfo[this.selectedModel].name == "Vicuna-7B-v1.1"){
                ...
                this.selectedMethod = ["LLM-Attacks"];
            } else {
                this.showmethodInfo = this.methodInfoNoParam;
            }
        ...
        updateMethod(index, checked){
            ...
            if (checked == true){
                if (this.selectedMethod.indexOf(methodId) === -1) {
                    this.selectedMethod.push(methodId);
                }
            }else{
                const idx = this.selectedMethod.indexOf(methodId);
                if (idx > -1) {
                    this.selectedMethod.splice(idx,1);
