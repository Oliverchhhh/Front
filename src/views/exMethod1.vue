        ...
        <div class="paramCon">
          ...
          <div class="moduleSwitch">
            <a-radio-group
              ...
              size="large"
            >
              <a-radio-button value="attribution">图像归因与特征降维解释</a-radio-button>
              <a-radio-button value="multi-modal">LIME多模态黑盒模型解释</a-radio-button>
            </a-radio-group>
          </div>
          <div class="funcParam">
            <div class="paramTitle">
              ...
              <a-button
                v-if="mode==='attribution'"
                class="DataEva"
                @click="dataEvaClick"
                :style="buttonBGColor"
                :disabled="disStatus"
              >
                <a-icon type="security-scan" />评估
              </a-button>
              <a-button
                v-if="mode==='multi-modal'"
                class="DataEva"
                @click="limeEvaClick"
                :style="buttonBGColor"
                :disabled="disStatus"
            ...
            <a-divider />
            <div v-if="mode==='attribution'" class="inputdiv">
              <!-- 输入主体 -->
              <div class="mainParamNameNotop">请选择数据集</div>
              <DataSetCard
                style="width: 1104px; margin-bottom: 16px;"
                v-for="(info, index) in dataSetInfo"
                :key="'Dataset' + index"
                v-bind="info"
                :indexInParent="index"
              ...
              <!-- <div style="width: 1104px;margin-bottom: 16px;">
                <a-upload action="/ex/uploadModel" name="ex_upload_model" @change="uploadModel">
                  <div class="uploadModelStyle">请上传模型</div>
                </a-upload>
              </div>-->
              <ModelCard
                style="width: 1104px; margin-bottom: 16px;"
                v-for="(info, index) in modelInfo"
                :key="'Model' + index"
                v-bind="info"
                :indexInParent="index"
              ...
              <a-row
                :gutter="16"
                style="margin-top:16px"
                v-for="(methods, i) in featureMethodInfo"
                :key="'feature'+i"
                type="flex"
              >
                <a-col :flex="24 / methods.length" v-for="(method, j) in methods" :key="j">
                  <a-button
                    :id="'feature' + i + j"
                    class="methodButton"
                    @click="changeFeatureMethods(i,j)"
                    @mouseover="featureMethodButtonOver(i, j)"
                  >{{ method.name }}</a-button>
                </a-col>
                <div
                  v-if="featureMethodHoverIndex==i"
                  class="attackmethodDes"
                >{{ featureMethodDescription }}</div>
              </a-row>
              <!-- 降维 -->
              ...
              <a-row
                :gutter="16"
                style="margin-top:16px"
                v-for="(methods, i) in dimensionMethodInfo"
                :key="'dimension'+i"
                type="flex"
              >
                <a-col :flex="24 / methods.length" v-for="(method, j) in methods" :key="j">
                  <a-button
                    :id="'dimension' + i + j"
                    class="methodButton"
                    @click="changeDimensionMethods(i,j)"
                    @mouseover="dimensionMethodButtonOver(i, j)"
                  >{{ method.name }}</a-button>
                </a-col>
                <div
                  v-if="dimensionMethodHoverIndex==i"
                  class="attackmethodDes"
                >{{ dimensionMethodDescription }}</div>
              </a-row>
              <!-- 模型内部解释 -->
              ...
              <a-radio
                ...
                :disabled="selectedFeatureMethod.length==0 ? true:false"
                @click="layerExplainChange"
              >Guided-backpropagation</a-radio>
              <ExMethodEval
                :isShow="resultVisible"
                :result="result"
                :postData="postData"
                :mode="mode"
                @on-close="() => {resultVisible = !resultVisible}"
              ></ExMethodEval>
            </div>
            <div v-if="mode==='multi-modal'" class="inputdiv">
              <!-- 多模态 -->
              <!-- 选数据集 -->
              <div class="datasetSelected">
                <p class="mainParamName">请选择样本所属数据集</p>
                <a-radio-group class="colummRadio" @change="onDatasetChoiceChange">
                  <div class="matchedDes">
                    <a-radio :style="radioStyle" value="imagenet" :checked="limeDatasetChoice=='imagenet'">ImageNet</a-radio>
                    <p class="matchedMethodText">
                      <span>ImageNet数据集：</span>是ILSVRC竞赛使用的是数据集，由斯坦福大学李飞飞教授主导，包含了超过1400万张全尺寸的有标记图片，大约有22000个类别的数据。
                    </p>
                  ...
                  <div class="matchedDes">
                    <a-radio :style="radioStyle" value="sst2">SST2</a-radio>
                    <p class="matchedMethodText">
                      <span>SST2数据集：</span>(斯坦福情感树库)，单句子分类任务，包含电影评论中的句子和它们情感的人类注释。这项任务是给定句子的情感，类别分为两类正面情感（positive，样本标签对应为1）和负面情感（negative，样本标签对应为0）
                    </p>
                    <p class="matchedMethodText">样本示例：</p>
                    <div class="demoData" style="display:flex;flex-flow:column;">
                      <P>（积极情感）If you sometimes like to go to the movies to have fun , Wasabi is a good place to start .</P>
                      <p>（消极情感）A heavy reliance on CGI technology is beginning to creep into the series .</p>
                    </div>
                  </div>
                </a-radio-group>
              </div>
                  ...
                  <div v-if="data_mode==='image'">
                    ...
                    >
                      ...
                      <div v-else>
                        ...
                        <div clradioStyleass="ant-upload-text">上传图片</div>
                      </div>
                    </a-upload>
                  </div>
                  <div v-if="data_mode==='text'">
                    <el-input
                      v-model="limeinput"
                      :rows="4"
                      type="textarea"
              ...
              <div class="modelSelected">
                ...
                <a-radio-group @change="onAiModelChange">
                  <div class="modelDiv">
                    <a-radio
                      :style="modelStyle"
                      value="vgg16"
                      :disabled="['imagenet'].indexOf(limeDatasetChoice) > -1? false: true"
                    >VGG16</a-radio>
                  </div>
                  <div class="modelDiv">
                    <a-radio
                  ...
                  <div class="modelDiv">
                    <a-radio
                      ...
                      value="efficientent"
                      :disabled="['imagenet'].indexOf(limeDatasetChoice) > -1? false: true"
                    >EfficientNet_b0</a-radio>
                  </div>
                  <div class="modelDiv">
                    <a-radio
                      :style="modelStyle"
                      value="lstm"
                      :disabled="['sst2'].indexOf(limeDatasetChoice) > -1? false: true"
                    >LSTM</a-radio>
                  </div>
                  <div class="modelDiv">
                    <a-radio
              ...
              <div v-if="data_mode==='text'">
                <a-row
                  :gutter="16"
                  style="margin-top:16px"
                  v-for="(methods, i) in textmethodInfo"
                  :key="'textattack'+i"
                  type="flex"
                >
                  <a-col :flex="24 / methods.length" v-for="(method, j) in methods" :key="j">
                    <a-button
                      :id="'textbutton' + i + j"
                      class="methodButton"
                      @click="changetextMethods(i,j)"
                      @mouseover="textmethodButtonOver(i, j)"
                    >{{ method.name }}</a-button>
                  </a-col>
                  <div v-if="methodHoverIndex==i" class="attackmethodDes">{{ methodDescription }}</div>
                </a-row>
...
import ExMethodEval from "../views/dialog/ExMethodEval1.vue";
...
export default {
  ...
  data() {
    return {
      ...
      funcDesText: {
        ...
        /* 背景介绍 */
        backinfo:
          "通过研究各类对抗性攻击的生成与作用机理，即其误导模型的内在原因，并建立易于人类理解的解释方法，为安全性验证与防御策略构建等工作提供理论指导。",
        /* 亮点介绍 */
        highlight: [
          ...
          "多模态黑盒分析可视化：采用基于LIME的黑盒分析方法，可以支持任意类型的模型，因而可以同时支持文本和图像数据类型，达到多模态分析效果"
        ]
      },
      modeMsg: {
        "attribution": { ID: "attribution", name: "图像归因与特征降维解释" },
        "multi-modal": { ID: "multi-modal", name: "LIME多模态黑盒模型解释" }
      },
      mode: "attribution",
      /* 评估结果 */
      result: {},
      ...
      dataSetInfo: [
        {
          name: "ImageNet",
          description:
            "是ILSVRC竞赛使用的是数据集，由斯坦福大学李飞飞教授主导，包含了超过1400万张全尺寸的有标记图片，大约有22000个类别的数据。",
          classname: [
            "数字0",
            "数字1",
            "数字2",
          ...
          pictureSrcs: [
            [
              ...
              require("../assets/img/ImageNet0.png")
            ]
          ]
        },
        {
          ...
          description:
            "是由 Hinton 的学生 Alex Krizhevsky 和 Ilya Sutskever 整理的一个用于识别普适物体的小型数据集。一共包含 10 个类别的 RGB 彩色图 片：飞机（ airplane ）、汽车（ automobile ）、鸟类（ bird ）、猫（ cat ）、鹿（ deer ）、狗（ dog ）、蛙类（ frog ）、马（ horse ）、船（ ship ）和卡车（ truck ）。图片的尺寸为 32×32 ，数据集中一共有 50000 张训练圄片和 10000 张测试图片。",
          pictureSrcs: [
            [
        ...
        },
        {
          name: "MNIST",
          description:
            "是一个手写体数字的图片数据集，该数据集来由美国国家标准与技术研究所（National Institute of Standards and Technology (NIST)）发起整理，一共统计了来自250个不同的人手写数字图片，其中50%是高中生，50%来自人口普查局的工作人员。该数据集的收集目的是希望通过算法，实现对手写数字的识别。",
          classname: [
            "数字0",
            "数字1",
            "数字2",
        ...
        [
          {
            name: "FGSM",
            description:
              "Fast Gradient Sign MethodFGSM快速梯度符号法是一种简单而有效的生成对抗样本的方法，其工作方式如下：在给定输入数据后，利用已训练的模型输出预测并计算损失函数的梯度，然后使用梯度的符号来创建使损失最大化的新数据。"
          },
          {
            name: "RFGSM",
            description:
              "Random Fast Gradient Sign MethodR-FGSM随机快速梯度符号法是FGSM的一种变体，在应用FGSM产生的对抗扰动之前，给在输入样本中增加一个小的随机扰动，这样有助于避免梯度Mask的防御策略。"
          },
          {
            name: "FFGSM",
            description:
              "FGSM in fast adversarial trainingFFGSM快速FGSM是FGSM的一种变体，在应用FGSM产生的对抗扰动前，给在输入样本中增加一个小的随机扰动。与R-FGSM不同的是，扰动以均匀分布代替高斯分布。"
          },
          {
            name: "MIFGSM",
            description:
              "Momentum Iterative Fast Gradient Sign MethodMI-FGSM基于动量的迭代FGSM是在I-FGSM的基础上，通过将动量项整合到攻击的迭代过程中，使计算结果能摆脱局部最优，并且增加了更新方向的稳定性。"
          }
        ],
        [
          {
            name: "DIFGSM",
            description:
              "Diverse Inputs Iterative Fast Gradient Sign Method(DI2-FGSM)输入多样的迭代FGSM是在I-FGSM的基础上，让输入图片随机进行数据增强，即在样本输入模型之前，以一定概率p对其进行随机resize，padding等操作。该方法增强了攻击的鲁棒性，使得黑盒攻击成功率显著增加。"
          },
          {
            name: "BIM",
            description:
              "Basic Iterative MethodBIM迭代式FGSM是FGSM方法的变体，其工作原理为：每轮迭代在上一步算得的对抗样本基础上，各像素增加（或减少）一个常数。"
          },
          {
            name: "EOTPGD",
            description:
              "Expectation Over Transformation based PGDEOTPGD变换期望PGD是将EOT的思想加入到PGD算法中，即迭代中用损失函数梯度的期望代替符号梯度本身。"
          },
          {
            name: "PGD",
            description:
              "Projected Gradient DescentPGD投影梯度下降法是FGSM的迭代版本，与BIM不同的是，它对每次迭代的结果进行裁剪，保证新样本的各个像素都在x的ϵ邻域（L∞）内。"
          },
          {
            name: "PGDL2",
            description:
              "L2-bounded PGDPGDL2L2范数投影梯度下降法是PGD算法的另一个版本，其对每次的迭代结果采用L2范数裁剪。"
          }
        ]
      ],
      textmethodInfo: [
        [
          {
            name: "PWWS",
            description:
              "PWWS(概率加权词显著性的贪婪算法)属于文本对抗领域的一种比较优秀的攻击方法。单词的替换基于同义词，同时还考虑了替换后分类概率的变化程度和每个词的显著性两个因素，攻击效果比较好。"
          },
          {
            name: "IGA",
            description:
              "IGA(改进的遗传算法)取消了句子中每个单词只能修改一次的限制。允许将单词替换为初始单词，这也避免了局部最优。在保证对抗样本可传递性的前提下实现更高的成功率。"
          },
          // {
          //   name:"HotFlip",
          //   description:
          //   "HotFlip基于单词中字符的原子翻转操作（例如：交换、插入、删除等）来构造对抗样本，同时通过语义约束控制对抗样本的隐蔽性，攻击效率很高。"
          // }
        ],
      ],
      selectedMethods: [],
        ...
        [
          {
            ...
            id: "lrp",
            description:
              "LRP算法：通过自定义模型反向传输规则，将模型决策的结果归因到样本像素中，进而可视化其中对分类有利和对分类有害的显著区域。"
          },
          {
            name: "Grad-CAM",
            id: "gradcam",
            description:
              "Grad-CAM算法：通过解构模型特征提取层的特征图（feature map），提取对模型当前决策起到积极影响的区域。"
          },
          {
        ...
        [
          {
            ...
            id: "pca",
            description:
              "PCA算法：主成分分析（Principal Component Analysis，PCA）是一种数学方法，用于将高维数据转化为低维表示，同时保留数据中最重要的信息。它通过寻找数据中的主要变化方向（主成分），将原始特征转化为这些主成分的线性组合，从而实现数据的降维和压缩。"
          },
          {
            name: "t-SNE",
            id: "tsne",
            description:
              "t-SNE算法：非线性降维算法，非常适用于高维数据降维到2维或者3维，并进行可视化。"
          },
          {
            name: "SVN",
            id: "svm",
            description:
              "SVN算法：采用支持向量机来拟合样本分布，并计算样本到SVM边界的距离和样本数据之间的关系，分析样本的分布情况。"
          }
        ]
      ],
      ...
      postData: {},
      uploadflag: false,
      limeDatasetChoice: "imagenet",
      limeModelChoice: "vgg16",
      loading: false,
      data_mode: "image",
      limeinput: ""
    };
  ...
  methods: {
    ...
    moduleChange(e) {
      ...
      this.selectedDimensionMethod = [];
      this.selectedFeatureMethod = [];
      this.selectedMethods = [];
      this.subModel = 0;
      this.layerchecked = false;
      // 恢复按钮的未选中样式
      for (let i in this.methodInfo) {
        for (let j in this.methodInfo[i]) {
    ...
    onDatasetChoiceChange(e) {
      // 修改选择数据集
      console.log("radio checked", e.target.value);
      this.limeDatasetChoice = e.target.value;
      if (this.limeDatasetChoice == "imagenet") {
        this.data_mode = "image";
    ...
    onAiModelChange(e) {
      console.log("radio checked", e.target.value);
      this.limeModelChoice = e.target.value;
    },
    /* 关闭结果窗口 */
    closeDialog() {
    ...
    getData() {
      var that = this;
      that.$axios
        .get("/output/Resultdata", { params: { Taskid: that.tid } })
        .then(data => {
    ...
    getLog() {
      ...
      that.$axios
        .get("/Task/QueryLog", { params: { Taskid: that.tid } })
        .then(data => {
    ...
    dataEvaClick() {
      ...
      if (
        this.selectedFeatureMethod.length == 0 &&
        this.selectedDimensionMethod.length == 0
      ) {
        this.$message.warning(
          "请至少选择一项特征归因可视化算法或数据分布降维可视化算法！",
          3
        );
        return;
      }
      ...
      var that = this;
      that.$axios
        .post("/Task/CreateTask", { AttackAndDefenseTask: 0 })
        .then(result => {
          console.log(result);
          ...
          // 特征归因和降维
          that.$axios
            .post("/Attack/AdversarialAnalysis", that.postData)
            .then(res => {
              that.stidlist["adversarial_analysis"] = res.data.stid;
    ...
    limeEvaClick() {
      ...
      if (this.data_mode == "image" && this.imageUrl == "") {
        this.$message.warning("请上传jpg/png样本图片！ ", 3);
        return 0;
      }
      if (this.data_mode == "text" && this.limeinput == "") {
        this.$message.warning("请输出文本样本！", 3);
        return 0;
      }
      if (this.limeDatasetChoice == "") {
        this.$message.warning("请选择数据集！", 3);
        return 0;
      }
      if (this.limeModelChoice == "") {
      ...
      that.$axios
        ...
        .then(result => {
          ...
          /* 请求体 postdata*/
          this.postData = {
            DatasetParam: { name: that.limeDatasetChoice, data: this.limeinput },
            ModelParam: { name: that.limeModelChoice, ckpt: null },
            AdvMethods: that.selectedMethods,
            Taskid: that.tid,
            mode: that.data_mode
          };
          this.logtext = [];
          that.logflag = true;
          that.$axios
            .post("/Attack/AttackLime", this.postData)
            .then(res => {
              console.log(res.data)
              that.stidlist["attack_lime"] = res.data.stid;
              that.clk = window.setInterval(() => {
                that.update();
              }, 60000);
    ...
    changeMethods(i, j) {
      ...
      if (button.style.color == "") {
        ...
        this.selectedMethods.splice(
          this.selectedMethods.indexOf(this.methodInfo[i][j].name),
          1
        );
      }
    },
    changetextMethods(i, j) {
      // debugger
      let button = document.getElementById("textbutton" + i + j);
      if (button.style.color == "") {
        this.methodHoverIndex = i;
        this.methodDescription = this.textmethodInfo[i][j].description;
        button.style.color = "#0B55F4";
        button.style.borderColor = "#C8DCFB";
        button.style.background = "#F2F4F9";
        this.selectedMethods.push(this.textmethodInfo[i][j].name);
      } else {
        this.methodHoverIndex = -1;
        this.methodDescription = "";
        button.style.color = "";
        ...
        button.blur();
        this.selectedMethods.splice(
          this.selectedMethods.indexOf(this.textmethodInfo[i][j].name),
          1
        );
      }
    },
    ...
    changeFeatureMethods(i, j) {
      ...
      if (button.style.color == "") {
        ...
        this.featureMethodDescription = this.featureMethodInfo[i][
          j
        ].description;
        button.style.color = "#0B55F4";
        button.style.borderColor = "#C8DCFB";
        ...
        button.style.background = "#F2F4F9";
        button.blur();
        this.selectedFeatureMethod.splice(
          this.selectedFeatureMethod.indexOf(this.featureMethodInfo[i][j].id),
          1
        );
    ...
    changeDimensionMethods(i, j) {
      ...
      if (button.style.color == "") {
        ...
        this.dimensionMethodDescription = this.dimensionMethodInfo[i][
          j
        ].description;
        button.style.color = "#0B55F4";
        button.style.borderColor = "#C8DCFB";
        ...
        button.style.background = "#F2F4F9";
        button.blur();
        this.selectedDimensionMethod.splice(
          this.selectedDimensionMethod.indexOf(
            this.dimensionMethodInfo[i][j].id
          ),
          1
        );
      }
