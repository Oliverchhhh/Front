...
<template>
    <div>
       ...
       <a-layout-content>
               ...
               <div class="funcParam">
                   ...
                   <div class="inputdiv">
                       ...
                       <div class="mainParamNameNotop">请选择数据集</div>
                        ...
                        <div class="mainParamName48">请选择自动化攻防方案</div>
                        <DefenseCard style="width: 1104px; " v-for="(info, index) in DefenseInfo" @selectedDefense="updateDefense"
                            :key="'Defense' + index" v-bind="info" :indexInParent="index"
                            :checked="selectedDefense == DefenseInfo[index].id"> 
                        </DefenseCard>
                        <div v-if="selectedDefense == 'flow'">
                        ...
                        <div v-if="['graph','graph_rule'].indexOf(selectedDefense)>-1">
                            <div class='attackMode'>
                                <p class="mainParamName48">请选择攻击方式</p>
                                <a-radio-group v-model="attackMode">
                                    <a-radio :style="radioStyle" value="white_box" >
                                        白盒攻击
                                    </a-radio>
                                    <a-radio :style="radioStyle" value="black_box" >
                                        黑盒攻击
                                    </a-radio>
                                </a-radio-group >
                            </div>
                            <div class='attackType'>
                                <p class="mainParamName48">请选择攻击类型</p>
                                <a-radio-group v-model="attackType">
                                    <a-radio :style="radioStyle" value="evasion_attack" >
                                        逃逸攻击
                                    </a-radio>
                                    <a-radio :style="radioStyle" value="poison_attack" >
                                        毒化攻击
                                    </a-radio>
                                </a-radio-group >
                            </div>
                            <div class='dataType'>
                                <p class="mainParamName48">请选数据类型</p>
                                <a-radio-group v-model="dataType">
                                    <a-radio :style="radioStyle" value="image" >
                                        图片
                                    </a-radio>
                                    <a-radio :style="radioStyle" value="text" >
                                        文本
                                    </a-radio>
                                    <a-radio :style="radioStyle" value="graph" >
                                        图
                                    </a-radio>
                                </a-radio-group >
                            </div>
                            <div class='defenseAlg'>
                                <p class="mainParamName48">请选择防御算法</p>
                                <a-select default-value="Adversarial-Training" :value="defendAlgorithm" style="width: 500px" @change='changeDefenseAlg'>
                                    <a-select-option v-for="i in defenseList" :key="i">
                                        {{i}}
                                    </a-select-option>
                                </a-select>
                            </div>
                        </div>
                        <div v-if="['rule','flow_rule'].indexOf(selectedDefense)>-1">
                            <div class="mainParamName48">请选择攻击方法（可多选）</div>
                            <div v-for="(methods, i) in ruleMethodInfo" :key="i" style="margin-bottom: 16px;">
                                <a-row :gutter="16" style="height:50px;" type="flex">
                                    <a-col :flex="24 / methods.length" v-for="(method, j) in methods" :key="j" class="denfenseMethod">
                                        <a-button :id="'button' + i + j"  @click="changeRuleMethods(i,j)"
                                            >{{ method.name }}</a-button>
                                    </a-col>
                                </a-row>
                                <div v-if="methodHoverIndex==i && methodDescription !== ''" style="padding:14px 24px;margin-bottom: 16px;"> {{ methodDescription }} </div>
           ...
           <!-- 结果展示 -->
           <autoAdvAttackEval :is-show="resultVisible" :result="result" :postData="postData" @on-close="() => { resultVisible = !resultVisible }">
            </autoAdvAttackEval>
       </a-layout-content>
       <a-layout-footer>
...
export default {
   ...
   data(){
       return{
        ...
        defenseList:[
            ...
            "InvGAN","SID", "ADC-detector","DIP","SmsNet","Activation Clustering",
            "Neural Cleanse","Deep k-NN","ATMBPR","SAO","DAVE","APT", "FNCF",
            "AMR","BAT","TextFirewall", "RanMask","FGWS","Safer","AMDA", "DISP",
        ...
        ruleMethodInfo:[[
            {name:"FGSM",description:"Fast Gradient Sign MethodFGSM快速梯度符号法是一种简单而有效的生成对抗样本的方法，其工作方式如下：在给定输入数据后，利用已训练的模型输出预测并计算损失函数的梯度，然后使用梯度的符号来创建使损失最大化的新数据",},
            {name:"BIM", description:"Basic Iterative MethodBIM迭代式FGSM是对FGSM的改进方法，主要的改进有两点，其一是FGSM方法是一步完成的，而BIM方法通过多次迭代来寻找对抗样本；其次，为了避免迭代过程中出现超出有效值的情况出现，使用了一个修建方法严格限制像素值的范围",},
            {name:"RFGSM", description:"RFGSM算法：R+FGSM在FGSM中加入随机的步骤, 是一个在白盒设置下高效的能替代迭代攻击的方法",},
            {name:"FFGSM", description:"FFGSM算法：在使用FGSM攻击算法前加入随机初始化的扰动，经过实验发现基于FFGSM的对抗训练拥有高效性",},
            {name:"TIFGSM", description:"TIFGSM算法：Translation-Invarient FGSM将translation-invarient attack与FGSM结合，具体是将对抗目标调整为使像素平移后的对抗样本输出与真实标签的loss最大，同时使用卷积梯度的方法减小计算代价，使用该攻击方法拥有更强的迁移性",}
            ],
            ...
            {name:"MIFGSM",description:"MIFGSM算法：momentum iterative FGSM是一种使用momentum迭代梯度的方法，该方法在迭代梯度对抗攻击(如BIM)的基础上，累计每次梯度方向的速度向量作为momentum，每次对抗扰动不再直接使用梯度方向，转而采用momentum方向，从而稳定更新方向并避免局部极值，更好提高攻击迁移性",},
            {name:"DIFGSM", description:"DIFGSM算法：Diverse Inputs Iterative Fast Gradient Sign Method,通过创建多样的输入模式提高对抗样本的迁移性。做法是对输入的原图像以p的概率加上随机且可导的变换(transformation)，使用梯度的方法最大化模型对变换后的原图像的损失函数值从而得到对抗图像",},
            {name:"CW", description:"该方法的出发点是攻击比较有名的对抗样本防御方法-防御蒸馏(就防御蒸馏方法而言，它在基本的L-BFGS，FGSM攻击方法上表现本身就比较差)。对于寻找对抗样本过程中目标函数的设置将会极大的影响对抗样本的攻击效果，为此，通过目标函数的设定，在零范数，二范数和无穷范数的限制下分别设计了三种不同的寻找对抗样本的目标函数，这三种方法均可以绕过防御蒸馏的防御",},
            {name:"UPGD", description:"支持各种基于梯度的对抗性攻击选项的终极PGD。",},
            {name:"PGD", description:"Projected Gradient DescentPGD投影梯度下降法是FGSM的迭代版本，该方法思路和BIM基本相同，不同之处在于该方法在迭代过程中使用范数投影的方法来约束非法数据，并且相对于BIM有一个随机的开始噪声。支持L1 norm范数",}
            ],[
            {name:"TPGD",description:"TPGD算法：基于KL-Divergence loss的pgd攻击",},
            {name:"PGDL2", description:"Projected Gradient DescentPGD投影梯度下降法是FGSM的迭代版本，该方法思路和BIM基本相同，不同之处在于该方法在迭代过程中使用范数投影的方法来约束非法数据，并且相对于BIM有一个随机的开始噪声。支持L2 norm范数",},
            {name:"SparseFool", description:"SparseFool算法：利用决策边界的低均值曲率计算对抗稀疏(sparse)扰动",},
            {name:"AutoPGD", description:"一种基于L1范数正则化的对抗性攻击算法，通过迭代生成对抗样本，以欺骗机器学习模型并导致错误的预测结果。该算法利用L1范数正则化来控制对抗样本的扰动大小，使得扰动更加稀疏",},
            {name:"OnePixel", description:" One pixel对抗攻击算法的特点为只改变一个像素点即可实现攻击，是一种黑盒攻击",}
            ],[
            {name:"Square",description:"该方法的主体思路是使用优化理论中的随机搜索方法来寻找对抗噪声，这种随机搜索的方法很多黑盒攻击方法都采用过。随机搜索的总体框架大体相同，如下图所示。基本思路是在在一个采用空间中随机选择一个噪声，如果该噪声能够降低目标函数的损失则将其添加到原图片上，否则进入下一轮的随机搜索。支持Linf norm范数。",},
            {name:"DeepFool", description:"DeepFool方法的出发点是想要精确的度量模型对于对抗样本的鲁棒性，为此提出了鲁棒性定义和计算方法。最终使用该计算方法生成对抗样本",},
            {name:"FAB", description:"FAB算法：Fast Adaptive Boundary Attack使用类似DeepFool投影到决策平面的方法，主要不同的是，FAB将过程中的扰动尽可能偏向原样本点，从而可以找到能改变分类器输出的最小扰动。扰动的衡量距离为 linf",},
            {name:"Jitter", description:"Jitter算法：使用新的损失函数从而改善效率和攻击成功率，同时通过输出logits标准化到固定值范围将规模不变性引入损失函数",},
            {name:"MIFGSM",description:"MIFGSM算法：momentum iterative FGSM是一种使用momentum迭代梯度的方法，该方法在迭代梯度对抗攻击(如BIM)的基础上，累计每次梯度方向的速度向量作为momentum，每次对抗扰动不再直接使用梯度方向，转而采用momentum方向，从而稳定更新方向并避免局部极值，更好提高攻击迁移性",},
            ],
        ...
        funcDesText:{
            ...
            bgimg:bgimg,
            /* 功能介绍下的总介绍 */
            destext:"自动化生成特定场景下的AI模型对抗测试方案",
            /* 背景介绍 */
            backinfo:"基于对抗样本攻击和防御算法之间的攻防关系，自动化生成特定场景下的AI模型对抗测试方案，评估AI模型鲁棒性。",
            /* 亮点介绍 */
            highlight:[
                "支持10类任务类型，如图片分类、目标检测、情感分析、机器翻译、3D点云、智能推荐、情感分析、语音识别、入侵检测、物体分割等机器学习任务",
                "支持20多种攻防算法，如常见的FGSM、BIM、RFGSM、CW、PGD、TPGD、MIFGSM等攻击算法，InvGAN、SID、Defense-GAN等主流的防御算法",
                "支持支持基于知识图谱的AI模型自动化攻防、基于流水线的AI模型自动化攻防、规则驱动的AI模型自动化攻防等5种安全自动化攻防技术",
            ]
        },
        /* 结果弹窗状态信息 */
        resultVisible:false,
   ...
   methods: { 
        ...
        // 防御方法点击选中
        changeRuleMethods(i, j) {
            // debugger;
            let button = document.getElementById("button" + i + j)
            if (button.style.color == "") {
                this.methodHoverIndex = i
                this.methodDescription = this.ruleMethodInfo[i][j].description
                button.style.color = "#0B55F4"
                button.style.borderColor = "#C8DCFB"
                button.style.background = "#F2F4F9"
                this.selectedRuleMethod.push(this.ruleMethodInfo[i][j].name)
                this.selectedAttributes[this.ruleMethodInfo[i][j].name] = {}
            } else {
                this.methodHoverIndex = -1
                this.methodDescription = ""
                button.style.color = ""
                ...
                button.style.background = "#F2F4F9"
                button.blur()
                this.selectedRuleMethod.splice(this.selectedRuleMethod.indexOf(this.ruleMethodInfo[i][j].name), 1 )
                delete this.selectedAttributes[this.ruleMethodInfo[i][j].name]
            }
        },
       // 更换数据集
       changeDataset(index) {
       ...
       updateDefense(index, checked){
        this.selectedDefense = this.DefenseInfo[index].id
      ...
      exportResult(){
       ...
       changeDefenseAlg(value){
        this.defendAlgorithm = value
        console.log("defendAlgorithm",this.defendAlgorithm)
       },
       initParam(){
            this.logtext=[]
            this.percent=0
       ...
       dataEvaClick(){
            ...
            var that=this;
            // that.tid = "20231026_1349_70fb68a"
            // that.postData["DatasetParam"] = {"name":dataset}
            // that.postData["ModelParam"] = {"name":model,"ckpt":null}
            // that.postData["AutoMethod"] = that.selectedDefense
            // that.postData["AdvMethods"] = that.selectedRuleMethod
            // that.postData["Taskid"] = that.tid
            // that.stidlist =  {"AutoAttack":"S20231026_1349_ef006b4"}
           ...
           this.$axios.post("/Task/CreateTask",{AttackAndDefenseTask:0}).then((result) => {
                ...
                if (that.selectedDefense == 'flow'){
                    if(that.selectedMethod.length == 0){
                        that.$message.warning('请至少选择一项对抗攻击方法！',3);
                        return
                    }
                    // 检查各个方法的参数
                    for(let i = 0; i < that.selectedMethod.length; i ++){
                        if (!(that.selectedMethod[i] in that.selectedAttributes)){
                            let attributes_dict = {}
                            let info =[]
                            for( let methodnum = 0 ; methodnum < that.methodInfo.length; methodnum++){
                                if (that.methodInfo[methodnum].id == that.selectedMethod[i]){
                                    info = that.methodInfo[methodnum].attributes
                                    break
                                }
                            }
                            ...
                            that.postData[that.selectedMethod[i]] = attributes_dict
                        }else{
                            that.postData[that.selectedMethod[i]] = that.selectedAttributes[that.selectedMethod[i]]
                        }
                    }
                    /* 请求体 postdata*/
                    that.postData["DatasetParam"] = {"name":dataset}
                ...
                else if(['graph','graph_rule'].indexOf(that.selectedDefense)>-1){
                    that.postData["DatasetParam"] = {"name":dataset}
                    that.postData["ModelParam"] = {"name":model,"ckpt":null}
                    that.postData["AutoMethod"] = that.selectedDefense
                    that.postData["Taskid"] = that.tid
                    that.postData['attack_mode'] = that.attackMode
               ...
               that.logclk = self.setInterval(that.getLog, 300);
               that.$axios.post("/AutoAttack", that.postData).then((res) => {
                   that.logflag = true;
                   // 异步任务
                   that.stidlist =  {"AutoAttack":res.data.stid}
